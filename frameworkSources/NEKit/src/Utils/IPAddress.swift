...
public func < (lhs: IPAddress, rhs: IPAddress) -> Bool {
    ...
    case (.IPv6(var addrl), .IPv6(var addrr)):
        ...
        let ret = withUnsafeBytes(of: &a) { ptrl -> UInt32 in
            let ii =  withUnsafeBytes(of: &addrr) { ptrr -> UInt32 in
                let r = memcmp(ptrl.baseAddress!, ptrr.baseAddress!, MemoryLayout.size(ofValue: addrl))
                return UInt32(r)
            }
